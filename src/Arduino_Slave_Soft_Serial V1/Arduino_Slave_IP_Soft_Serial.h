...
class Arduino_Slave_IP_Soft_Serial
{
	public:
		Arduino_Slave_IP_Soft_Serial(SoftwareSerial* soft_serial);
...
Arduino_Slave_IP_Soft_Serial::Arduino_Slave_IP_Soft_Serial(SoftwareSerial* soft_serial)
{
...
void Arduino_Slave_IP_Soft_Serial::fullHanlde()
{
	if(_soft_serial->available() > 0)
	{
		...
		{
		  _soft_serial->write(built_response_IP_ADU[i]);
		}
		// response message sending ------------------------------------
